...
using codium.Runtime.OP;
using codium.Runtime.OperandModels;
using codium.Runtime.Variables;

namespace codium.Runtime.ScriptEngine
{
    /// <summary>
    ...
    public class ScriptContext
    {
        ...
        private void ProcessRelationalInstruction()
        {
            ...
            if (typeDest == typeof(int))
                ...
                fValueDest = (float)objectValueDest;
            else if (typeDest == typeof(long))
                fValueDest = (long)objectValueDest;
            else
                throw new ExecutionException(
                    "Values of type '" + typeDest.Name
            ...
            if (typeSource == typeof(int))
                ...
                fValueSource = (float)objectValueSource;
            else if (typeSource == typeof(long))
                fValueSource = (long)objectValueSource;
            else
                throw new ExecutionException(
                    "Values of type '" + typeSource.Name
                    + "' cannot be used in relational instructions.");
